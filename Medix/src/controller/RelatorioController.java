...
public class RelatorioController {
    ...
    public int totalInternacoesAtivas() {
        int n = 0;
        for (Internacao i : internacaoRepo.findAll()) {
            if (i.estaAtiva()) {
                n++;
            }
        } return n;
    ...
    public double faturamentoEstimado() {
        double total = 0.0;
        for (Consulta c : consultaRepo.findAll()) {
            total += precoLiquidoConsulta(c);
        } return total;
    }

    public double faturamentoNoPeriodo(String deInclusive, String ateInclusive) {
        double total = 0.0;
        for (Consulta c : consultasNoPeriodo(deInclusive, ateInclusive)) {
            total += precoLiquidoConsulta(c);
        } return total;
    }
}
